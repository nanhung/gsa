...
EP.plt<-function(X1, X2, colval, main){
  ...
  sen[sen < 0] <- NA # remove log value less than 1
  sen<-na.omit(sen)
  ins<-na.omit(ins)
  sen.ecdf <- ecdf(sen)
  ins.ecdf <- ecdf(ins)
...
EP.plt(sim1,sim2,3, "Percent cumulative")
EP.plt(sim1,sim2,6, "Percent cumulative")
EP.plt(sim3,sim4,9, "Percent cumulative")
